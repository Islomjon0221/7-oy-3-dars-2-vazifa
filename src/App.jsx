  ...
  return (
    <div className='container bg-[#0d0714] text-white w-full h-full mx-auto'>
      ...
      <div className="wrapper_card w-[432px] mb-[60px] gap-[17px] mx-auto flex flex-col">
        <h3 className='text-[16px] leading-[19.36px]'>Tasks to do - {counterTask}</h3>
        {
          user.counter.map((todo, i) => {
            return (
              <div key={i} className='cards py-[22px] px-[21px] rounded-[10px] items-center flex justify-between bg-[#15101c]'>
                <h3 className='text-[#9E78CF] text-[16px] leading-[19.36px]'>{todo.username}</h3>
                <div className='w-[68px] h-[30px] flex justify-between'><img className='cursor-pointer transition duration-200 hover:opacity-[50%]' src={check} alt='check-icon' onClick={() => handleCheck(todo.username, todo.id)} /> <img className='cursor-pointer transition duration-200 hover:opacity-[50%]' onClick={() => handleDelete(todo.id)} src={trash} alt="trash-icon" /></div>
              </div>
            )
          })
        }
      ...
      <div className="done_card w-[432px] pb-[363px] gap-[17px] mx-auto flex flex-col">
        <h3 className='text-[16px] leading-[19.36px]'>Done - {counterDone}</h3>
        {
            ...
            return (
              <div key={i} className='cards py-[22px] px-[21px] rounded-[10px] items-center flex justify-between bg-[#15101c]'>
                <h3 className='text-[#78CFB0] line-through text-[16px] leading-[19.36px]'>{todo.username}</h3>
                <img className='cursor-pointer transition duration-200 hover:opacity-[50%]' width={22} height={22} onClick={() => handleReturn(todo.id, todo.username)} src={returnIcon} alt="return-icon" />
              </div>
            )
          })
        }
